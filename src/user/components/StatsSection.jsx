...
const StatsSection = ({ totalHashrate, activeMiners, pricePerTHs }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
                { label: t('Hashrate Total Pool', 'Total Pool Hashrate'), value: `${totalHashrate.toFixed(2)} TH/s`, color: 'from-blue-400 to-cyan-300', bg: 'bg-blue-500/10', icon: '⚡' },
                { label: t('Mineros en la Red', 'Miners in Network'), value: activeMiners, color: 'from-green-400 to-emerald-300', bg: 'bg-green-500/10', icon: '🖥️' },
                { label: t('Precio por TH/s (USD)', 'Price per TH/s (USD)'), value: `$${pricePerTHs.toFixed(2)}`, color: 'from-orange-400 to-yellow-300', bg: 'bg-orange-500/10', icon: '💲' },
